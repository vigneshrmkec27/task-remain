...
const Dashboard = ({ user, darkMode, setDarkMode, showNotification, onUserUpdate }) => {
    ...
    const priorityData = [
        { label: 'High', value: tasks.filter((task) => task.priority === 'HIGH').length, color: 'from-rose-500 to-orange-400' },
        { label: 'Medium', value: tasks.filter((task) => task.priority === 'MEDIUM').length, color: 'from-amber-400 to-yellow-300' },
        { label: 'Low', value: tasks.filter((task) => task.priority === 'LOW').length, color: 'from-emerald-500 to-teal-400' },
    ];
    const statusData = [
        { label: 'Pending', value: tasks.filter((task) => task.status === 'PENDING').length, color: 'from-slate-500 to-slate-400' },
        { label: 'In Progress', value: tasks.filter((task) => task.status === 'IN_PROGRESS').length, color: 'from-sky-500 to-indigo-400' },
        { label: 'Completed', value: tasks.filter((task) => task.status === 'COMPLETED').length, color: 'from-emerald-500 to-lime-400' },
    ];
